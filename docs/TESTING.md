...
mkdir -p .fixtures
docker compose run --rm --no-deps --user root \
    -v "$PWD/scripts/make_live_ui_fixture.py":/tmp/mk.py -v "$PWD/.fixtures":/out \
...
markers, the rejected quotations and the model record. It makes no Bedrock call.

### Checking a real model, once credentials exist

With live mode configured (see the README), confirm the server offers it and run one live run from
the UI on the three fictional demo documents. Record the model ID, region, token usage, candidate
count and any rejected quotations in `BUILD_STATUS.md`. Do not describe a fixture result as a live
result.

## CI

The included GitHub Actions workflow runs container tests and builds/starts the app, then exercises the HTTP/restart script. It does not require AWS credentials or paid inference. It is provided source, **not a claim that a remote GitHub run has passed**. UI automation remains a separate local acceptance step. `apps/web/package-lock.json` is now committed, so the frontend image builds with `npm ci`.
